...
pub struct Group {
    ...
    pub propagate_member_visibility_access: bool,
    pub name: IS,
    pub description: IS,
    pub deleted: bool,
    pub logo_id: Uuid,
    ...
    async fn for_filters(&self, user: User) -> MinilithResult<Json<Vec<Group>>> {
        let groups = sqlx::query_as!(
            ...
            r#"select distinct
                ...
                pg.propogate_member_visibility_access as "propagate_member_visibility_access!",
                pg.name as "name!: DIS",
    ...
    async fn joinable_groups(&self, user: User) -> MinilithResult<Json<Vec<JoinableGroup>>> {
        let groups = sqlx::query!(
            r#"select distinct
                ...
                target.propogate_member_visibility_access as "propagate_member_visibility_access!",
        ...
        .map(|row| JoinableGroup {
            group: Group {
                ...
                propagate_member_visibility_access: row.propagate_member_visibility_access,
                name: row.name.0,
