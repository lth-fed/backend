...
For making the database types work in the CI, please run `cargo sqlx prepare` whenever you add new queries or migrations.

> This is because in the CI we don't have a database, so sqlx needs to know the type of the queries beforehand, which `cargo sqlx prepare` does by writing files about the queries.
